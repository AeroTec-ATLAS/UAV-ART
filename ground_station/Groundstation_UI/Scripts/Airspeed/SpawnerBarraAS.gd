...
func _process(_delta):
	...
	#print(contBarraM)
	if(estado == 1):
		if (float(Debug.x) >= (160.5/2 * (contBarraM_AS-1))): #161.164 #por ajuste da alt ini
			#print("criou1")
	...
	#		contBarraN -= 1
		...
		if (float(Debug.x) >= (136.0/2 * (contBarraM_AS-1))): #137 #InitialAlt.pinit * 10 + 10 * (SpawnerNum.contM -3)
			#print("criou2")
			#print("criou2")
			#print(contBarraM)
