function [curve, W] = long_curve_fitting(xdata, y, beta0, t) 

figure()
plot(t, y, 'ro');

%Complete model 
F = @(beta, xdata)beta(1)*xdata(:,1)+ beta(2)*xdata(:,2) + beta(3)*xdata(:,3) + beta(4)*xdata(:,4);
...
%Complete model 
y_fitted = curve(1)*xdata(:,1) + curve(2)*xdata(:,2)+ curve(3)*xdata(:,3)+ curve(4)*xdata(:,4);
