...
function sys=mdlDerivatives(t,x,uu,P)
    ...
    p_vec = getInertialVelocities(phi,theta,psi,u,v,w);
    pndot = p_vec(1); pedot = p_vec(2); pddot = p_vec(3);
    udot = Fx/P.mass + r*v - q*w;
    vdot = Fy/P.mass + p*w - r*u;
    wdot = Fz/P.mass + q*u - p*u;
    a_vec = getAngleDerivatives(phi,theta,p,q,r);
    phidot = a_vec(1); thetadot = a_vec(2); psidot = a_vec(3);
    [pdot,qdot,rdot] = getAngularRateDerivatives(p,q,r,L,M,N);
