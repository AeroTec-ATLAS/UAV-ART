...
function [X,Y] = pointsAlongDubinsPath(dubinspath,Del)
  ...
  th2 = mod(atan2(dubinspath.z1(2)-dubinspath.cs(2),dubinspath.z1(1)-dubinspath.cs(1)),2*pi);
  if dubinspath.lambda_s>0
      if th1>=th2
  ...
  sig = 0;
  while sig<=1
      X = [X; (1-sig)*dubinspath.z1(1) + sig*dubinspath.z2(1)];
      Y = [Y; (1-sig)*dubinspath.z1(2) + sig*dubinspath.z2(2)];
      sig = sig + Del;
  end
    
  % points along end circle
  th2 = mod(atan2(dubinspath.pe(2)-dubinspath.ce(2),dubinspath.pe(1)-dubinspath.ce(1)),2*pi);
  th1 = mod(atan2(dubinspath.z2(2)-dubinspath.ce(2),dubinspath.z2(1)-dubinspath.ce(1)),2*pi);
  if dubinspath.lambda_e>0
      if th1>=th2
        th = [th1:Del:2*pi,0:Del:th2];
      else
        th = [th1:Del:th2];
