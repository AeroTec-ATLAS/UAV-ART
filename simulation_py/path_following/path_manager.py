...
class pathManager:
    ...
    def dubins_manager(self, waypoints, radius, state):
        ...
        GetDubins = self.compute_parameters(waypoints.ned[:,self.ptr_previous], 
                                            ...
                                            waypoints.course[self.ptr_current], 
                                            radius)
        if self.manager_state == 1:
            ...
            if np.vdot(mav_pos.T-GetDubins[5], -GetDubins[6])>=0:
            ...
            self.path.line_origin = self.path.line_origin.reshape(3,1)
            self.path.line_direction = GetDubins[6]#q1
            self.path.line_direction = self.path.line_direction.reshape(3,1) 
            if np.vdot(mav_pos.T-GetDubins[7], GetDubins[6])>=0:
        ...
        if self.i == 5000:
            ...
            print("Origin", self.path.line_origin,"\n CenterOrbit", self.path.orbit_center)
            print("\n\n")
            #print(GetDubins)
            self.i=0
    ...
    def compute_parameters(self, ps, chis, pe, chie, R):
        ...
        if ell < 2 * R:
            ...
            cre = pe + R*np.array([np.cos(chie + pi2), np.sin(chie + pi2), 0 ])
            cle = pe + R*np.array([np.cos(chie - pi2), np.sin(chie - pi2), 0 ])
            ...
            if idx == 0:
                ...
                w1 = cs + R*(np.dot(self.rotz(-pi2),q1)).T
                w2 = ce + R*(np.dot(self.rotz(-pi2),q1)).T
            elif idx == 1:
                cs = crs
                lams = 1    
                ce = cle
                ...
                w1 = cs + R*(np.dot(self.rotz(v+v2),e1)).T
                w2 = ce + R*(np.dot(self.rotz(v+v2-np.pi),e1)).T
            elif idx == 3:
                cs = cls
                lams = -1
                ce = cle
            ...
            dubinsParams = np.array([L,cs,lams,ce,lame,w1,q1,w2,w3,q3.T])
