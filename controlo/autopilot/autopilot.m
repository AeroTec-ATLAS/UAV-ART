...
% 
% The template of this file is available in the repository in Github of the
% book authors.
% 
...
%   bz       = uu(19+NN); % z-gyro bias
    ...
    [delta, x_command] = autopilot_uavbook(Va_c,h_c,chi_c,Va,h,chi,phi,...
                                                    theta,beta,p,q,r,t,P);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% autopilot_uavbook
%   - autopilot defined in the uavbook (check chapter VI)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [delta, x_command] = autopilot_uavbook(Va_c,h_c,chi_c,Va,h,...
                                            chi,phi,theta,beta,p,q,r,t,P)
...
%         delta_r = 0; % the aircraft is designed to drive the sideslip to 0
        delta_a = roll_hold(phi_c, phi, 1, p, P.G.k_p_roll, ...
                                ...
                                -P.G.delta_a_max, P.G.delta_a_max, P.Ts);
    else
...
%         delta_r = 0;
    ...
    switch altitude_state
        case 1  % in take-off zone 
            ...
            theta_c = P.theta_c_climb*(1-exp(-t)); %include pre-filter
            
        case 2  % climb zone
            delta_t = P.throttle_take_off;
            theta_c = airspeed_pitch_hold(Va_c, Va, flag, ...
                                            P.G.k_p_pitch_airspeed, ...
                                            P.G.k_i_pitch_airspeed, ...
                                            -pi/6, pi/6, P.Ts);
        ...
        case 4 % altitude hold zone
            delta_t = airspeed_throtle_hold(Va_c, Va, P.u_trim(4), ...
                                    flag, P.G.k_p_throttle_airspeed, ...
                                    P.G.k_i_throttle_airspeed, 0 , 1, P.Ts); 
                                
            theta_c = altitude_pitch_hold(h_c, h, flag, ...
                                            P.G.k_p_pitch_altitude, ...
                                            P.G.k_i_pitch_altitude, ...
                                            -pi/6, pi/6, P.Ts);            
    end
